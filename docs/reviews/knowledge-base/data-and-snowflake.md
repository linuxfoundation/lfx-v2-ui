...
**Empirical citation:** general pattern surfaced in CodeRabbit comments on PRs touching `snowflake.service.ts` callers. The `lfx-skills:lfx-self-serve-code-reviewer` subagent also flags this in its Snowflake direct-SQL check, but pre-PR catching it is cheaper.
