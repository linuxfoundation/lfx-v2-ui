...
**Detect:** in any custom SSE parser (`parseUpstreamSSEBlock` in `apps/lfx-one/src/server/services/copilot.service.ts`, or the frontend parser in `apps/lfx-one/src/app/shared/services/sse.service.ts`), find the line-handling logic for `data:` lines. Verify it concatenates (`data = (data ? data + '\n' : '') + raw`) rather than reassigns (`data = raw`). Also verify the `TextDecoder` is `decode()`-flushed after the read loop (multibyte boundaries) and the SSE split delimiter is `\n\n` (event-block separator).
