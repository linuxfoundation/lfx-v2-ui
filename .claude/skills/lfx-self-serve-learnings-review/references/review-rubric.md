...
2. **Severity is per-pattern, not per-category.** Each pattern in a `<file>.md` declares its own default severity. The category is for routing/loading, not severity assignment.
...
3. **Empirical observation** — review of CodeRabbit + Copilot comments on ~30 PRs from this repo over a 2-month window (2026-03-15 to 2026-05-15). Patterns that appeared in ≥2 PRs were canonicalised into `*.md`.
