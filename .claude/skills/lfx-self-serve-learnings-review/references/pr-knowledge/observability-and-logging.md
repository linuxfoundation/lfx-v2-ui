...
**Read when:** `apps/lfx-one/otel.mjs`, `apps/lfx-one/src/server/middleware/rate-limit.ts`, any new route registration in `server.ts`, or any file using `logger.info` / `logger.warning` / `logger.debug`. Cross-checked by Phase 5.

---

## `pr-knowledge/observability-and-logging/otel-ignore-list-drift` — SHOULD_FIX
...
## `pr-knowledge/observability-and-logging/health-endpoint-inside-rate-limiter` — SHOULD_FIX
...
## `pr-knowledge/observability-and-logging/count-variable-mismatch` — SHOULD_FIX
...
## `pr-knowledge/observability-and-logging/info-for-high-frequency-fetch` — SHOULD_FIX
...
## `pr-knowledge/observability-and-logging/err-field-vs-error-message` — SHOULD_FIX
...
## `pr-knowledge/observability-and-logging/missing-route-in-tracer-config` — NIT
