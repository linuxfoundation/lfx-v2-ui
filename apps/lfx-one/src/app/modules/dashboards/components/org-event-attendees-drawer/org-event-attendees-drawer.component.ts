...
const DEFAULT_MONTHLY: OrgEventAttendeesMonthlyResponse = { monthlyData: [], monthlyLabels: [], totalAttendees: 0 };
  ...
  protected readonly attendanceChartOptions: ChartOptions<'bar'> = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        cornerRadius: 6,
        callbacks: {
          label: (ctx) => ` ${(ctx.parsed.y as number).toLocaleString()} attendees`,
        },
      },
    },
    scales: {
    ...
    return toSignal(
      toObservable(this.visible).pipe(
        switchMap((isVisible) => {
          ...
          return this.analyticsService.getOrgEventAttendeesMonthly(accountId, foundationSlug).pipe(
