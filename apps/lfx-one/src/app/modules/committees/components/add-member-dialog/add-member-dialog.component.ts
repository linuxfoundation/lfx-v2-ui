...
@Component({
  ...
  imports: [ReactiveFormsModule, NgClass, ButtonComponent, InputTextComponent, OrganizationSearchComponent, SelectComponent, CheckboxComponent, SkeletonModule],
...
export class AddMemberDialogComponent {
  ...
  /** Returns uppercase initials (max 2 chars) for avatar display. */
  public getInitials(user: UserSearchResult): string {
    const first = user.first_name?.charAt(0) ?? '';
    const last = user.last_name?.charAt(0) ?? '';
    return (first + last).toUpperCase() || user.email?.charAt(0).toUpperCase() || '?';
  }

  /** Returns a deterministic background color class for the avatar based on the user's email. */
  public getAvatarColor(user: UserSearchResult): string {
    const colors = ['bg-blue-500', 'bg-violet-500', 'bg-emerald-500', 'bg-amber-500', 'bg-rose-500', 'bg-indigo-500', 'bg-teal-500'];
    ...
    this.committeeService.createCommitteeMember(committee.uid, memberData).subscribe({
      next: () => {
        ...
        this.messageService.add({
          ...
          detail: `${user.first_name} ${user.last_name} has been added to the group.`,
        });
