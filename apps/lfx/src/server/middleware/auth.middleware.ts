...
async function executeAuthDecision(decision: AuthDecision, req: Request, res: Response, next: NextFunction): Promise<void> {
  switch (decision.action) {
    ...
    case 'error': {
      const error =
        decision.errorType === 'authorization'
          ? ApiError.forbidden('Insufficient permissions to access this resource')
          : ApiError.unauthorized('Authentication required to access this resource');
      next(error);
      break;
    }
  }
